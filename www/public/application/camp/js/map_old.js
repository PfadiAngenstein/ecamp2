...
{
var req = this.req;
if (req && !req.aborted && req.readyState == 4)
{
var data = null;
try
{
...
var newparams = $.isEmptyObject(params) ? [] : ($.isArray(params) ? params : parse_query($.param(params)));
var combinedparams = [];
var seen = {}, newhash = {};
...
return encodeURIComponent(v.name).replace(/%5B|%5D/gi, decodeURIComponent) + "=" + encodeURIComponent(v.value);
});
...
if (path.match(/^(https?:)?\/\/[^\/]*$/))
path = parts[1] + "/";
path = path.replace(/\\/g, '/').replace(encodechars, encodeURIComponent).replace(/'/g, '%27');
...
{
var locparts = document.location.pathname.match(/(\.\w\w)\.html$/);
var myhostparts = document.location.hostname.match('^[-a-z]+\.(.*)');
path = path.replace(/^(\/\/\w+)\//, "$1." + (myhostparts? myhostparts[1] : "") + "/");
return U(locparts ? path.replace(/\/($|\?)/, "/index.html" + "$1").replace(/(\.html|\.gif|\.jpg|\.png|\.json|\.php|\.js)($|\?)/, locparts[1] + "$1$2", path).replace(/index.html($|\?)/, "$1") : path, params);
}
function Q(value)
{
return typeof value == "undefined" ? "" : value.toString().replace(/&/g, '&amp;').replace(/\"/g, '&quot;').replace(/[<]/g, '&lt;').replace(/>/g, '&gt;');
}
L.Projection.SearchProjection = L.Class.extend({
options: {
base: [0, 0],
zoomlevel: 1
},
...
.subtract(this.options.base)
.divideBy(this.options.zoomlevel);
},
unproject: function(point) {
point = point
.multiplyBy(this.options.zoomlevel)
.add(this.options.base);
return L.latLng(point.y, point.x);
}
});
(function (LatLng) {
L.SearchPoint = function (lat, lng, alt) {
LatLng.call(this, lat, lng, alt);
this.x = this.lng;
this.y = this.lat;
};
L.SearchPoint.prototype = L.extend({}, LatLng.prototype, {
constructor: L.SearchPoint
});
L.LatLng = L.SearchPoint;
})(L.LatLng);
...
var marker_offset = [env.center.y, env.center.x];
var initial_center = (env.center.base_type == "request") ? $.extend({}, env.center) : this.dynamic_center(env.center);
if (env.center.pos) {
initial_center.x = env.center.pos.x;
initial_center.y = env.center.pos.y;
} else if (env.center.base_type == "telcloud" && env.geo_position && this.lz2zd(initial_center.lz) >= 512) {
initial_center.lz = this.zd2lz(4);
coords = this.wgs2swiss(env.geo_position.coords.longitude, env.geo_position.coords.latitude);
initial_center.x = coords[0];
initial_center.y = coords[1];
}
var map = this.map = L.map(mapcontainer, {
crs: L.extend({}, L.CRS.Simple, {projection: new L.Projection.SearchProjection({
base: [400000, 400000],
zoomlevel: 4096
})}),
zoom:initial_center.lz,
zoomControl:false,
fadeAnimation:false,
...
maxBounds: [[11000,421000],[360000,898000]],
...
map.setView([initial_center.y, initial_center.x], initial_center.lz);
...
var bglayer = this.bglayer = L.tileLayer('{mapserver}/chmap.jpg?layer={layer}&l={tileSize}:{z}/{y}/{x}{grayscale}', bgoptions).addTo(map);
...
var fglayer = this.fglayer = L.tileLayer('{mapserver}/chmap.png?layer={drawingsBelowText}{layer}{drawingsAboveText}&l={tileSize}:{z}/{y}/{x}&poi={poi}{telparams}&pixelratio={pixelratio}{drawing}{route}{poiadmin}',fgoptions).addTo(map);
...
markerlayer.setLatLng([env.center.marker.y, env.center.marker.x]);
markerlayer.addTo(map);
}
L.control.attribution({ position:'bottomleft', prefix:'©<a class="sl_state_link map-permalink" href="//map.search.ch/">search.ch</a>, TomTom, swisstopo, <a href="http://www.openstreetmap.org/copyright">OSM</a>' }).addTo(map);
L.Control.Scale.prototype._update = function() {
this._updateScales(self.get_zd() * this.options.maxWidth)
};
...
if (poi.dyn) {
poi.zoomvisible = zd <= poi.p.maxzoom && zd >= poi.p.minzoom;
...
markerlayer.poi = data;
var offset = [data.offset.y, data.offset.x];
markerlayer.setLatLng(offset).addTo(map);
self.infocard.html(data.pois).slideDown('fast');
self.set_poi_drawing(self.infocard.find('.sl-card-inner').data('drawing'));
},
...
y:   coord.y,
zd:  zd,
poi: self.poi.replace(/default/, env.poi_defaults[map.getZoom()]),
d:   env.center.drawing,
lang: self.env.language
}) + env.center.coord_base + fglayer.options.poiadmin;
if (self.env.ua.browser == "MSIE" && self.env.ua.version < 10)
url += "&callback=?";
...
var active = poi.p.enabled && poi.visible && poi.zoomvisible;
poi.dist_sq = active ? (poi.dyn ? poi.squared_dist_to(coord.x, coord.y, zd) : Math.pow(poi.x - coord.x, 2) + Math.pow(poi.y - coord.y, 2)) : 0;
if (active && poi.dist_sq < 25 * zd_sq) {
...
activeDynpois = activeDynpois.sort(function(a, b) { return a.dist_sq - b.dist_sq; });
data.offset = {x:activeDynpois[0].x, y:activeDynpois[0].y};
data.pois = $.map(activeDynpois, function(poi, i) { return poi._card(i);}).join('')
+ (data.pois && !data.address ? data.pois : '');
}
this._update_poi_card(data);
...
url += "&callback=?";
self.infocard.slideUp('fast');
$.getJSON(url, this._update_poi_card);
}
});
...
_show: function (data) {
var pois = data.pois;
var maxdist_sq = Math.pow(this.hover_slack, 2);
var zd = self.get_zd();
var zd_sq = zd * zd;
var x = this.hover_coord.x;
var y = this.hover_coord.y;
var disp_pois = [];
function consider(poi) {
var dist_sq = poi.dyn ? poi.squared_dist_to(x, y, zd) : Math.pow(poi.x - x, 2) + Math.pow(poi.y - y, 2);
if (dist_sq <= (poi.dyn ? zd_sq * 25 : maxdist_sq)) {
poi.dist_sq = dist_sq;
disp_pois.push(poi);
}
}
$.each(self.dynpois, function(id, poi) {
if (poi.p.enabled && poi.visible && poi.zoomvisible)
consider(poi);
});
if (pois)
...
consider(pois[i]);
if (disp_pois.length) {
disp_pois = disp_pois.sort(function(a, b) { return a.dist_sq - b.dist_sq; });
var html = '<table class="map-poi-hover">' + $.map(disp_pois, function(poi) {
return '<tr><td>' + (poi.icon ? '<img src="' + (poi.dyn ? poi.icon : self.env.map_server + '/images/icons/17/' + poi.icon) + '">' : '') + '</td><td><span class="title">' + (poi.title ? poi.title : poi.html) + '</span> <span class="name">' + poi.name + '</span></td></tr>';
}).join('') + '</table><div class="map-poi-hover-tipp">' + env.T.poi_hover_tipp + '</div>';
if (html != this.hover_popup._content)
this.hover_popup.setLatLng([disp_pois[0].y, disp_pois[0].x]).setContent(html).openOn(this._map);
...
var zd = self.get_zd();
var grid  = 128 * zd;
this.hover_slack = 12 * zd * (zd <= 0.125 ? 2 : 1);
this.hover_coord = coord;
var url = LU(self.env.map_server + '/api/poi_html.json', {
mode: 'hover',
mx1:  Math.floor(coord.x / grid) * grid - this.hover_slack,
mx2:  Math.ceil( coord.x / grid) * grid + this.hover_slack,
my1:  Math.floor(coord.y / grid) * grid - this.hover_slack,
my2:  Math.ceil( coord.y / grid) * grid + this.hover_slack,
zd:   zd,
poi: self.poi.replace(/default/, env.poi_defaults[map.getZoom()]),
d:    env.center.drawing,
lang: self.env.language
...
zd2lz: function(zd)
{
return 12 - Math.round(Math.log(zd) / Math.log(2));
},
lz2zd: function(lz)
{
return Math.pow(2, 12 - lz);
},
get_zd: function()
{
return this.lz2zd(this.map.getZoom());
},
set_zd: function(zd)
{
this.map.setZoom(this.zd2lz(zd));
},
get_url_params: function() {
var c = this.map.getCenter();
return {x:'', y:'', pos: Math.round(c.x) + ',' + Math.round(c.y), z: 512 / this.get_zd()};
},
...
var offsets = this.offsets();
var div_width  = $(this.mapcontainer).innerWidth() - offsets.left - offsets.right - 20;
var div_height = $(this.mapcontainer).innerHeight() - offsets.top - offsets.bottom - 20;
for (var lz = this.minZoom; lz <= this.maxZoom; lz++) {
var zd = this.lz2zd(lz);
if (center.dimensions[0] <= div_width * zd && center.dimensions[1] <= div_height * zd)
center.lz = lz;
}
}
if (center.dxm)
...
this.map.setView([dyncenter.y, dyncenter.x], dyncenter.lz, p || { animate:false });
this.setMarker(center);
},
setMarker: function(center)
{
if (center.marker)
this.markerlayer.setLatLng([center.y, center.x]).addTo(this.map);
else
...
options.drawing = env.center.drawing && !('drawing_objects' in env.center) ? '&d=' + env.center.drawing : '';
...
add_poi: function(offset, poi)
{
poi.map = this.map;
poi.layer = L.marker(offset, {
icon:L.icon({
iconUrl:poi.p.icon,
...
if (!poi.p.maxzoom)
poi.p.maxzoom = this.lz2zd(this.minZoom);
if (!poi.p.minzoom)
poi.p.minzoom = this.lz2zd(this.maxZoom);
var zd = this.get_zd();
poi.zoomvisible = zd <= poi.p.maxzoom && zd >= poi.p.minzoom;
...
$.getJSON(
this.env.map_server + "/api/?action=getcenter" + (this.env.ua.browser == "MSIE" && this.env.ua.version < 10 ? "&callback=?" : "") + "&lang=" + this.env.lang,
params,
...
return L.geoJson(geojson, $.extend({},
{ coordsToLatLng: function(c) { return new L.LatLng(c[1], c[0]); }},
options
)
);
}
...
this.opened = false;
this.p = { enabled:true, html:'', icon:'https://map.search.ch/images/icons/17/p0.gif' };
...
this.img.onerror = function(){ alert("Error loading icon " + this.src + ", using default icon."); this.src = _poi.icon = _poi.p.icon = "https://map.search.ch/images/icons/21/p1.gif"; };
this.img.src = this.p.icon;
}
if (this.map_api && (updates.icon || updates.center)) {
...
if (self.map_api) {
self.map_api.map_loaded(function() {
var l = self.map_api.l;
l.update_poi_card({
offset: {
x:self.coords[0],
y:self.coords[1]
...
self._trigger_popupopen({});
if (!self.isVisible()) {
self.map_api.go({center: self.coords});
}
});
}
});
...
close: function()
{
var l = this.map_api && this.map_api.l;
if (l && l.infocard) {
...
isVisible: function()
{
if (this.map && this.coords && this.visible && this.zoomvisible) {
var bounds = this.map.getBounds();
return bounds.contains(this.coords);
}
else
return false;
},
...
zoomupdate: function(vis)
{
if (this.icon)
it_dom[(vis&&this.visible?'show':'hide')](this.icon);
this.zoomvisible = vis;
},
squared_dist_to: function(x, y, zd)
{
var bbox = [
this.x - this.offsetx * zd,
this.y - this.offsety * zd,
this.x + (this.iwidth - this.offsetx) * zd,
this.y + (this.iheight - this.offsety) * zd
];
...
return 0;
if (insideXbounds)
return Math.pow(y <= bbox[1] ? bbox[1] - y : y - bbox[3], 2);
if (insideYbounds)
return Math.pow(x <= bbox[0] ? bbox[0] - x : x - bbox[2], 2);
function sqdist(px, py) {
return Math.pow(px - x, 2) + Math.pow(py - y, 2);
}
return Math.min(
sqdist(bbox[0], bbox[1]),
sqdist(bbox[0], bbox[3]),
sqdist(bbox[2], bbox[1]),
sqdist(bbox[2], bbox[3])
);
},
triggerEvent: function(evt, e)
{
...
SearchChPOI.prototype._triggerEvent = SearchChEvent.prototype.triggerEvent;
...
styles: {
'default': [
{color: '#022bb1', opacity: 0.7, weight: 6},
{color: 'white',   opacity: 0.2, weight: 4}
],
'hotline': [
{opacity: 0.7, weight: 6, smoothFactor: 0, gradient: false}
],
'debug_default': [
{color: '#ff0000', opacity: 0.6, weight: 6},
{color: 'white',   opacity: 0.2, weight: 4}
],
'debug_unpaved': [
{color: '#ff0000', opacity: 0.6, weight: 6, dashArray: '15,5', lineCap: 'butt'},
{color: 'white',   opacity: 0.2, weight: 4, dashArray: '15,5', lineCap: 'butt'}
],
'unpaved': [
{color: '#022bb1', opacity: 0.6, weight: 6, dashArray: '15,5', lineCap: 'butt'},
{color: 'white',   opacity: 0.2, weight: 4, dashArray: '15,5', lineCap: 'butt'}
],
car_forbidden: [
{color: '#cc000a', opacity: 0.6, weight: 6},
{color: 'white',   opacity: 0.2, weight: 4}
],
walk: [
{color: '#1b6c00', opacity: 0.6, weight: 6},
{color: 'white',   opacity: 0.2, weight: 4}
],
missingRouteStyles: [
...
this._addSegment(
points,
this.options.styles[route.segments[i].style] || this.options.styles['default'],
true,
route.segments[i].style == "hotline"
);
for (var j = 0; j < points.length; j++) {
var p = points[j];
if (last)
dist += this._distance(last, p);
...
d = this._distance(point, this._route.coordinates[i]);
if (d < minDist) {
minIndex = i;
minDist = d;
}
...
if (this._distance(wpLatLng, routeCoord) >
this.options.missingRouteTolerance) {
this._addSegment([wpLatLng, routeCoord],
this.options.styles.missingRouteStyles);
}
}
},
_addSegment: function(coords, styles, mouselistener, hotline) {
var i, max, min,
pl;
...
max = Math.max(coords[j].alt, max);
pl = L.hotline(coords, $.extend({className:'route-path', min:0, max:max}, styles[i]));
} else
pl = L.polyline(coords, $.extend({className:'route-path'}, styles[i]));
this.addLayer(pl);
if (mouselistener) {
...
return;
if ('search_poi_click' in this._map && this._map.search_poi_click._enabled
&& !('_route' in this._map && this._map._route._onDynPOI(e.latlng, this.options.l.get_zd()))
) {
var map = this._map;
...
latLng = new L.LatLng((1-a) * prev[1].y + a * cur[1].y, (1-a) * prev[1].x + a * cur[1].x, (1-a) * prev[1].alt + a * cur[1].alt);
}
return latLng;
},
_onShowPosition: function(e) {
if (!$(e.currentTarget).parents('.map-route-summary.active').length || $('.map-route-form:visible input[type="submit"].sl-spinner').length)
return;
...
if (i)
dist += this._distance(points[i-1], points[i]);
...
var tooltip = $('#map-charthover-tooltip');
...
{
if (!i)
return this.p.card.replace(/sl-card-body/, 'sl-card-body active');
return this.p.card;
},
triggerEvent: function(evt, e) {},
_trigger_popupopen: function(e) {},
...
L.Routing.poi = function(options) {
return new L.Routing.POI(options);
};
...
riseOnHover:true,
icon: L.icon({
iconUrl: this._env.map_server + '/images/map_route_' + (!i ? 'start' : (i == n-1 ? 'end' : 'via')) + (this._env.ua.svg ? '.svg' : '.png'),
iconSize:    [25, 35],
iconAnchor:  [12, 35],
popupAnchor: [1, -34],
shadowSize:  [41, 41],
...
marker = this._createMarker(newWpIndex, wp, this._waypoints.length + 1),
lines = [],
mouseMove = L.bind(function(e) {
var i;
if (marker) {
...
var path = route.geometry[i];
var points = path[0] == "hotline" ? this._polyline_decode(path[1], 1, 3) : this._polyline_decode(path[1], 1, 2);
segments[i] = {style:path[0], points:points};
coordinates = coordinates.concat(points);
}
}
var wps = $.map(route.waypoints, function(p) { return L.Routing.waypoint(new L.LatLng(p.y, p.x), p.text);});
return {
env:            this._env,
segments:       segments,
...
data.icon = data.icon;
data.id = id;
var poi = L.Routing.poi(data);
this.options.l.add_poi([data.y, data.x], poi);
this.pois[id] = poi;
}, this));
},
...
address: p.center,
points:  $.map(wps, function(p) { return p.name ? p.name : p.latLng.x + ',' + p.latLng.y }),
...
_polyline_decode: function(str, precision, tuple) {
var index = 0,
i = 0,
coordinates = [],
shift = 0,
...
point[i] += ((result & 1) ? ~(result >> 1) : (result >> 1));
}
coordinates.push(new L.LatLng(point[1] / factor, point[0] / factor, point[2] / factor, point[3] / factor));
}
return coordinates;
},
...
if (lastPos && (Math.abs(coord.x - lastPos.x) > 100 * zd || Math.abs(coord.y - lastPos.y)) > 100 * zd)
this._tooltip.remove();
setTimeout(L.bind(function() {
if (!this._last_hover_pos || Math.abs(coord.x - this._last_hover_pos.x) > 10 * zd || Math.abs(coord.y - this._last_hover_pos.y) > 10 * zd)
return;
if (this._map)
...
var poi = dynpois[i];
if (   poi.x - poi.offsetx * zd <= coord.x
&& poi.x + poi.offsetx * zd >= coord.x
&& poi.y - poi.offsety * zd <= coord.y
&& poi.y + poi.offsety * zd >= coord.y
)
return true;
}
...
{
includes: SearchChEvent.prototype,
initialize: function(p)
{
...
S.Leaflet.prototype.initialize.call(this, {
...
var center = this.env.center;
this.triggerEvent('load', center ? {mx:center.x, my:center.y} : {});
...
var zd = this.get_zd();
this.triggerEvent('change', {zoom:zd, zd:zd });
}
}, this);
...
this.map.on('dblclick mousedown mouseup mouseover mouseout mousemove zoomstart zoomend dragstart dragend maptypechanged contextmenu', function(e) {
this.triggerEvent(e.type, e);
}, this);
...
if (p.zoom) {
this.env.center.lz = this.zd2lz(p.zoom);
...
if ('grayscale' in p) {
this.bglayer.options.grayscale = p.grayscale ? '&grayscale=1' : '';
this.bglayer.redraw();
}
}.bind(this));
},
...
this._getcenter(p, function(center) {
this.ready(function() {
var coords = [center.y, center.x];
if (p.zoom)
this.map.setView(coords, this.zd2lz(p.zoom));
else
...
p.dx *= $(this.mapcontainer).width();
if (p.dy && p.dy <= 1 && p.dy >= -1)
p.dy *= $(this.mapcontainer).height();
var dxm = (p.dx ? p.dx : 0) * zd;
var dym = (p.dy ? p.dy : 0) * zd;
var centerCoords = this.map.getCenter();
this.env.center.x = centerCoords.x + dxm;
this.env.center.y = centerCoords.y - dym;
this.env.center.lz = p.zoom ? this.zd2lz(p.zoom) : this.map.getZoom();
this.map.setView([this.env.center.y, this.env.center.x], this.env.center.lz);
if (p.marker) {
this.env.center.marker = p.marker;
this.setMarker(this.env.center);
}
...
x: shift.x + 'm',
y: shift.y + 'm',
mx: this.l ? this.map.getCenter().x : 0,
my: this.l ? this.map.getCenter().y : 0,
poigroups: this.poi,
circle: marker,
marker: marker,
...
poi.map_api = this;
var self = this;
function addPOIatCoords(coords) {
var x = poi.x = coords.x;
var y = poi.y = coords.y;
poi.coords = [y, x];
self.api_dynpois[poi.id] = poi;
self.ready(function() {
poi.icon_ready(function() {
self.add_poi(poi.coords, poi);
setTimeout(function() {
...
if (continuing) {
this.map.on('click', function(e) {
callback({x:e.latlng.x, y:e.latlng.y});
});
} else {
...
poi = this.env.poi_defaults[this.map.getZoom()];
var pois = [];
var pset = p.split(',');
var current = it_array_flip(poi.split(','));
...
if (groups[k])
it_set(current, it_array_flip(groups[k].split(',')));
});
...
return SearchChEvent.prototype.triggerEvent.call(this, evt, e);
},
...
y:          y,
base_type:  'geo',
coord_base: '&base=' + this.env.basex + ',' + this.env.basey,
drawing:''
});
return;
}
...
var self = this;
$.getJSON(
this.env.map_server + "/api/" + (this.env.ua.browser == "MSIE" && this.env.ua.version < 10 ? "?callback=?" : ""),
{
action:  'getcenter',
address: p.center,
points:  p.points,
...
var c = this.map.getCenter();
shift.x = Math.round(c.x - this.env.center.x);
shift.y = Math.round(c.y - this.env.center.y);
}
return shift;
},
...
top:  $(this.mapcontainer).offset().top + 4,
left: $(this.mapcontainer).offset().left + 4,
'max-height': $(this.mapcontainer).height() - 21,
...
var a = $(row1).children('td:eq(' + col + ')').text().toLowerCase();
var b = $(row2).children('td:eq(' + col + ')').text().toLowerCase();
a = a.match('[^0-9]') ? a : parseInt(a);
b = b.match('[^0-9]') ? b : parseInt(b);
return  a > b ? 1 : (a < b ? -1 : 0);
}).appendTo(table);
thead.children('th').removeClass('sort-asc');
$(this).addClass('sort-asc');
});
SearchChMap.prototype.baseenv = {map_server:"http://map.search.ch",
basex:485000,
basey:296000,
maxtilesize:640,
language:"de",
type:"low",
poi_defaults:["", "", "", "", "", "", "", "", "", "", "", "bergbahn,haltestelle,parking,viasuisse,zug", "bergbahn,haltestelle,parking,viasuisse,zug", "all", "all", "all"],
additional_poi:"",
poi:"",
niceurl:"",
T:{sl_current_location:"Aktueller Standort",
locationerror:"Die aktuelle Position konnte nicht bestimmt werden.",
coordinates_outside_map:"Koordinaten liegen ausserhalb des Kartenbereichs",
mapmoveto:"Karte zu diesem Punkt bewegen",
fromcenter:"von der Kartenmitte",
poi_hover_tipp:"Für Details aufs Symbol in Karte klicken",
zoomin:"Reinzoomen",
zoomout:"Wegzoomen",
routedrag_tooltip:"Zum Ändern der Route ziehen",
routedrag_maxvia:"Maximale Anzahl via-Punkte erreicht",
close:"Schliessen",
sl_altitude_unit:" m.ü.M.",
confirmreset:"Wollen Sie die Änderungen wirklich verwerfen und zur letzten gespeicherten Version zurückkehren?",
notsaved:"Die Zeichnung enthält noch nicht gespeicherte Änderungen.",
coordinates:"Koordinaten",
map_bandwidth_high:"Luftbild",
map_bandwidth_low:"Karte"},
max_vias:6,
oev_completion_url:"//fahrplan.search.ch/api/completion.json?mapmode=1",
hideads:0,
geojson:"",
ua:{browser:"Firefox",
version:"57.0",
...
date:"",
base_type:"default",
coord_base:"&base=485000,296000",
lz:3,
dimensions:[335000, 210000],
infocard:"",
messages:""},
css:["http://map.search.ch/api/itjs/?files=itjs%2Fsearchlib.css,searchlib-full.css,leaflet.css,map.css,oev.css,map-full.css,oev-full.css,map_api.css&mapapi=1&c=7e8d934a0e"],
alltypes:["business", "person", "business_p", "zug", "haltestelle", "bergbahn", "parking", "viasuisse", "mobility", "veloland", "bar", "cafe", "hotel", "restaurant", "kino", "museum", "theater", "tour", "feuerstelle", "spielplatz", "bad", "camping", "kirche", "polizei", "schule", "spital", "verwaltung", "apotheke", "geldautomat", "post", "shop", "tankstelle", "ladestation", "berg", "pass", "wasserfall", "webcam", "wikipedia"],
poicombine:{'apotheke,bad,bar,berg,bergbahn,business,business_p,cafe,camping,feuerstelle,geldautomat,haltestelle,hotel,kino,kirche,ladestation,mobility,museum,parking,pass,person,polizei,post,restaurant,schule,shop,spielplatz,spital,tankstelle,theater,tour,veloland,verwaltung,viasuisse,wasserfall,webcam,wikipedia,zug':"all",
'business,business_p,person':"tel",
'bergbahn,haltestelle,mobility,parking,veloland,viasuisse,zug':"verkehr",
'bar,cafe,hotel,restaurant':"gastro",
'bad,camping,feuerstelle,kino,museum,spielplatz,theater,tour':"kultur",
'kirche,polizei,schule,spital,verwaltung':"gebaeude",
'apotheke,geldautomat,ladestation,post,shop,tankstelle':"service",
'berg,pass,wasserfall,webcam,wikipedia':"geo"}};
